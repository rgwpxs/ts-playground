...
class CustomNode {
    public key: number;
    public val: number;
    public next: CustomNode | null;
    public pre: CustomNode | null;
    constructor(key?: number, val?: number, pre: CustomNode | null = null, next: CustomNode | null = null) {
        this.key = key;
        this.val = val;
        this.next = next;
        this.pre = pre;
...
class LRUCache {
    private head: CustomNode = new CustomNode();
    private last: CustomNode | null = this.head;
    private arr: (CustomNode | undefined)[] = [];
    ...
    get(key: number): number {
        console.log('get', key);
        if (typeof this.arr[key]?.val !== 'undefined') {
            const oldNode = this.arr[key];
            if (this.last === oldNode) {
                return oldNode.val;
            }
            const oldNext = oldNode.next; // 85
            const oldPre = oldNode.pre;  // 96
            this.last.next = oldNode; // 3
            oldNode.next = null;
            oldNode.pre = this.last;
            oldPre.next = oldNext;
            oldNext.pre = oldPre;
            this.last = oldNode;
            log(this.head);
    ...
    put(key: number, value: number): void {
        ...
        if (typeof this.arr[key] !== 'undefined') {
            ...
            const oldPre = this.arr[key].pre;
            const oldNext = this.arr[key].next;
            this.last.next = oldNode;  
            oldPre.next = oldNext;
            oldNext.pre = oldPre;
            oldNode.pre = this.last;
            oldNode.next = null;
...
for (let i = 1; i < test.length; i++) {
    const o = test[i];
    console.log('command', i, 'of', test.length);
    if (i === 418) {
        debugger
    }
    if (o.length === 2) {
        ...
        let r = lru.get(o[0]);
        if (r !== expected[i]) {
            console.log('failed at command', i, 'key', o[0], 'expected', expected[i], 'result', r);
            debugger
        }
    }
}
...
function log(head) {
    ...
    const arr: string[] = [];
    while(last) {
        arr.push(`${last.pre?.key}-${last.key}-${last.next?.key}`);
        last = last.next;
    }
